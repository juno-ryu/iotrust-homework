...
import Swiper from "@/app/_components/client/swiper/swiper";
...
export default function Home() {
  ...
  return (
    <main className="font-sans flex flex-col gap-[32px] items-center min-h-screen">
      <div className="w-full max-w-lg min-w-[320px] px-4 py-10">
        <Swiper slides={slides} />
      </div>
      <div className="w-full max-w-lg min-w-[320px] flex flex-col gap-[32px] p-4">
        {/* <section className="w-full">
          <h3 className="text-lg font-bold">즐겨찾기</h3>
          <div className="w-full min-h-[1px] bg-gray-600 my-2" />
          {list.map((item) => (
            <Fragment key={`favorite_${item.id}`}>
              <div className="flex w-full gap-[20px]">
                <div className="flex shrink-0 w-[80px] h-[80px] bg-white rounded-lg overflow-hidden shadow-md shadow-gray-600">
